...
| **Indicator** | **Purpose** |
| --- | --- |
| RSI | Identifies overbought (&gt;70) or oversold (&lt;30). |
| MACD | Tracks trend/momentum via EMA differences. |
| Bollinger Bands | Measures volatility with SMA and deviations. |
| Moving Average Crossover | Signals trend shifts with short/long MAs. |
| Volume Analysis | Confirms trends with volume data. |
| Stochastic Oscillator | Predicts reversals via price range comparison. |
| Williams %R | Detects overbought/oversold levels. |
| Ichimoku Cloud | Visualizes support, resistance, and trends. |
| ADX | Gauges trend strength (&gt;25 = strong). |
| CCI | Spots cyclical price trends. |
| Parabolic SAR | Marks potential price reversal points. |
...
| --- | --- | --- | --- |
| X_API | Xeggex API key | String | Required |
| X_SECRET | Xeggex API secret | String | Required |
| NODE_ENV | Mode (`development`/`production`) | String | development |
| PORT | Server port | Number | 3000 |
| HOST | Server host | String | localhost |
| LOG_LEVEL | Logging level | String | info |
| TRADING_ENABLED | Enable live trading | Boolean | false |
| PAPER_TRADING | Enable paper trading | Boolean | true |
| MAX_POSITIONS | Max open positions | Number | 3 |
...
- **Request Body**:
  ...
  ```json
  {"symbol": "BTC/USDT", "side": "buy", "amount": 0.01, "price": 65100.00}
  ```
...
- **Request**:
  ...
  ```bash
  curl -X POST http://localhost:3000/api/trade -H "Content-Type: application/json" -d '{"symbol":"BTC/USDT","side":"buy","amount":0.01,"price":65100.00}'
  ```
...
- **Example**:
  ...
  ```javascript
  const data = await fetchMarketData('BTC/USDT', '1h');
...
- **Example**:
  ...
  ```javascript
  const rsi = calculateRSI([65000.50, 65150.30, ...], 14);
  // Output: 65.43
  ```
...
- **Example**:
  ...
  ```javascript
  const trade = await executeTrade('BTC/USDT', 'buy', 0.01, 65100.00);
...
- **API Errors**: Logged via Winston (HTTP 500).
- **Trade Errors**: Invalid parameters return HTTP 400.
- **Data Errors**: Falls back to cached data in `data/cache/`.
- **Logging**: Errors saved to `logs/` for debugging.
...
- **Credentials**: Store `X_API` and `X_SECRET` securely in `.env`.
- **Rate Limits**: Adhere to Xeggex API limits to avoid bans.
